...
def prediction(Strokedataset):
    
    prediction = model.predict(Strokedataset)
...
def user_input_features():
    ...
    hypertension = st.sidebar.selectbox('Hypertension',('0','1'),key='hypertension')
    heart_disease = st.sidebar.selectbox('Heart Disease',('0','1'),key='heart_disease')
    ...
    features = pd.DataFrame(Strokedataset,index=[0])
    return features

input_df = user_input_features()
raw_data = pd.read_csv('Clean_Stroke.csv')
Strokedataset = raw_data.drop(columns=['stroke'])
df = pd.concat([input_df,Strokedataset],axis=0)
